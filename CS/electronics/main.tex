...
\begin{document}
    ...
    \begin{itemize}
        ...
        \item \textbf{op-apm (operational amplifier)}:  An operational amplifier, usually called an op-amp, is an electronic component that amplifies the difference between two input voltages.
            \bigbreak \noindent 
            It has two inputs
            \begin{itemize}
        ...
        \item \textbf{Closed-loop gain}: Most amplifier circuits connect the output back to an input. This is called feedback, and the resulting gain is called the closed-loop gain. Negative feedback sacrifices the op-amp’s enormous open-loop gain to obtain a smaller, stable, and predictable circuit gain.
            ...
            \begin{align*}
                A_{CL} = 10
            .\end{align*}
        \item \textbf{Non-inverting amplifier}: In a non-inverting amplifier, the signal enters the \(+\) input. Resistors provide feedback to the \(-\) input.
            \bigbreak \noindent 
            Negative feedback makes \(V_-\) approximately equal to \(V_+\) because the output is connected back to the inverting input in a way that opposes any difference between them.
            \bigbreak \noindent 
            Consider a voltage follower:
            \begin{itemize}
                \item Input signal is connected to \(V_+\).
                \item Output is connected directly to \(V_-\).
            \end{itemize}
            Thus,
            \begin{align*}
                V_{-} = V_{\text{out}}
            .\end{align*}
            Suppose $V_{+} = 2V$. If the output starts at 0V,
            \begin{align*}
                V_{-} = 0V
            .\end{align*}
            Now, $V_{+} > V_{-}$. The op-amp responds by increasing its output voltage. As the output rises, \(V_-\) also rises because they are connected.
            \bigbreak \noindent 
            If the output becomes too high, say 2.1V, then
            \begin{align*}
                V_{-} = 2.1V,\; V_{+} = 2V
            .\end{align*}
            So, $V_{-} > V_{+}$, the op-amp lowers its output. The circuit settles near the point in which
            \begin{align*}
                V_{-} \approx V_{+}
            .\end{align*}
        \item \textbf{The comparator circuit}: A comparator circuit compares two analog voltage levels and switches its output to a high or low digital state to show which voltage is higher
            \bigbreak \noindent 
